...
function CommonModal({ isDialogOpen, setIsDialogOpen, url, setIsFetch }) {
    ...
    userData?.fieldData.reduce((acc, field) => {
      if (
        ...
        acc[
          ...
          ) // Email validation using Yup's email method
  ...
  const isSaleOrPurchaseYarn =
    modelData?.page === "SaleYarn" ||
    modelData?.page === "PurchaseYarn" ||
    modelData?.page === "StockYarn";
        ...
        >
          <Form>
            ...
            <ModalFooter>
              ...
              {selected.isEdit ? (
                <Button
                  sx={{ background: "#5eaba2", color: "white" }}
                  className="submit-hover"
                  type="submit"
                  disabled={btnDisable}
                >
                  {modelData?.page == "StockYarn" ? "Sale" : "Edit"}
                </Button>
              ) : (
                <Button
                  sx={{ background: "#5eaba2", color: "white" }}
